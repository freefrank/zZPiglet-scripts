...
function Sign() {
    ...
    let param = '/?lang=' + $.lang + '&os_type=' + $.os_t + '&os_version=' + $.os_v + '&_time=' + time + '&version=' + '1.3.168' + '&device_id=' + $.d_id + '&heybox_id=' + $.h_id + '&hkey=' + hkey
    ...
    return $.get({
        ...
        headers: {
            ...
            'User-Agent': 'xiaoheihe/' + '1.3.168' + ' (iPhone; ' + $.os_t + ' ' + $.os_v + '; Scale/3.00)'
        }
    })
        .then((resp) => {
...
function GetCookie() {
    if (cookiereg.exec($request.headers['Cookie'])[1]) {
        ...
        if ($.read('pkey') != (undefined || null)) {
            if ($.read('pkey') != pkey || $.read('lang') != lang || $.read('os_t') != os_t || $.read('os_v') != os_v || $.read('d_id') != d_id || $.read('h_id') != h_id) {
                $.write(pkey, 'pkey')
                $.write(lang, 'lang')
                $.write(os_t, 'os_t')
                $.write(os_v, 'os_v')
